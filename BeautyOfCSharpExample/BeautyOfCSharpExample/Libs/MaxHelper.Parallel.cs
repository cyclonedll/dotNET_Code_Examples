...
public static partial class MaxHelper
{
    ...
    private static Task[] AllocateTasks_LocateMax<TValue>(
        ...
        out TValue[] maxValues, out Index[] maxValueIndics)
        where TValue : IComparable, IComparable<TValue>
    {
        var maxValuesForTasks = new TValue[workerCount];//结果集 每个任务的最大值

        var maxIndicsForTasks = new Index[workerCount];//结果集 每个任务的最大值的索引
    ...
    public static Task<(Index, TValue)> LocateMaxAsync<TValue>(this TValue[] values, int? numberOfWorkers = null, bool useTPL = false)
        where TValue : IComparable, IComparable<TValue>
    {
        if (useTPL)
        {
        ...
        else
        {
            return Task.Run(() =>
            {
                ...
                var tasks = AllocateTasks_LocateMax(workerCount, values, 0, values.Length, out TValue[] maxValues, out Index[] maxIndics);
    ...
    /// <returns></returns>
    public static Task<(Index, TValue)> LocateMaxAsync<TValue>(this TValue[] values, int start, int length,
        ...
        where TValue : IComparable, IComparable<TValue>
    {
        ...
        else
        {
            return Task.Run(() =>
            {
                ...
                var tasks = AllocateTasks_LocateMax(workerCount, values, start, length, out TValue[] maxValues, out Index[] maxIndics);
