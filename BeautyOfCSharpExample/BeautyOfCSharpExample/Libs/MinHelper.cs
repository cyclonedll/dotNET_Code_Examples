public static partial class MinHelper
{
    ...
    public static (Index, TValue) LocateMin<TValue>(this TValue[] values)
        where TValue : IComparable, IComparable<TValue>
    {
        TValue result = values[0];
        int resultIndex = 0;
    ...
    /// <returns></returns>
    public static (Index, TValue) LocateMin<TValue>(this TValue[] values, int start, int length)
        where TValue : IComparable, IComparable<TValue>
    {
        TValue result = values[start];
        int resultIndex = 0;
        ...
        where TValue : IComparable, IComparable<TValue>
    {
        ...
        {
            var lastSegmentLength = length - (int)localStartIndex2;
    ...
    private static Task[] AllocateTasks_LocateMin<TValue>(
        ...
        out TValue[] minValues, out Index[] minValueIndics)
        where TValue : IComparable, IComparable<TValue>
    {
        var minValuesForTasks = new TValue[workerCount];//结果集 每个任务的最大值

        var minIndicsForTasks = new Index[workerCount];//结果集 每个任务的最大值的索引
    ...
    public static Task<(Index, TValue)> LocateMinAsync<TValue>(this TValue[] values, int? numberOfWorkers = null, bool useTPL = false)
        where TValue : IComparable, IComparable<TValue>
    {
        if (useTPL)
        {
        ...
        else
        {
            return Task.Run(() =>
            {
                ...
                var tasks = AllocateTasks_LocateMin(workerCount, values, 0, values.Length, out TValue[] maxValues, out Index[] maxIndics);
    ...
    /// <returns></returns>
    public static Task<(Index, TValue)> LocateMinAsync<TValue>(this TValue[] values, int start, int length,
        ...
        where TValue : IComparable, IComparable<TValue>
    {
        ...
        else
        {
            return Task.Run(() =>
            {
                ...
                var tasks = AllocateTasks_LocateMin(workerCount, values, start, length, out TValue[] maxValues, out Index[] maxIndics);
